...
# 文件保存后缀
suffix = "pt-all"  # 可以修改为任何你想要的后缀，如 "_v2" 或 "_experiment1"

# 输出文件夹
output_folder = "ResNet34/pt-all"  # 可以修改为任何你想要的相对路径，如 "experiments/run1"
...
def detailed_test_evaluation():
    ...
    # 保存混淆矩阵图
    confusion_matrix_path = os.path.join(output_folder, f'confusion_matrix{suffix}.png')
    ...
    # 保存归一化混淆矩阵图
    normalized_cm_path = os.path.join(output_folder, f'confusion_matrix_normalized{suffix}.png')
    plt.savefig(normalized_cm_path, dpi=300, bbox_inches='tight')
    print(f"归一化混淆矩阵已保存为 {normalized_cm_path}")
    ...
    # 保存为numpy文件
    test_results_path = os.path.join(output_folder, f'detailed_test_results{suffix}.npy')
    np.save(test_results_path, evaluation_results)
    print(f"\n详细评估结果已保存为 {test_results_path}")
...
def plot_training_curves(train_losses, train_accuracies, val_losses, val_accuracies):
    ...
    # 保存图像
    training_curves_path = os.path.join(output_folder, f'training_curves{suffix}.png')
    ...
    plt.tight_layout()
    loss_curves_path = os.path.join(output_folder, f'loss_curves{suffix}.png')
    plt.savefig(loss_curves_path, dpi=300, bbox_inches='tight')
    print(f"Loss curves saved as {loss_curves_path}")
    plt.show()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    accuracy_curves_path = os.path.join(output_folder, f'accuracy_curves{suffix}.png')
    plt.savefig(accuracy_curves_path, dpi=300, bbox_inches='tight')
    print(f"Accuracy curves saved as {accuracy_curves_path}")
    plt.show()
...
def train_model():
    ...
    for epoch in range(EPOCHS):
        ...
        if log_val['val_accuracy'] > best_val_accuracy:
            ...
            # 保存最佳准确率模型
            model_path = os.path.join(output_folder, f'best_model{suffix}.pth')
            torch.save({
                'epoch': epoch,
            ...
            print(f"最佳准确率模型已保存为 {model_path}")
...
if __name__ == "__main__":
    ...
    # 保存训练历史
    np.save(os.path.join(output_folder, f'train_losses{suffix}.npy'), train_losses)
    np.save(os.path.join(output_folder, f'train_accuracies{suffix}.npy'), train_accuracies)
    np.save(os.path.join(output_folder, f'val_losses{suffix}.npy'), val_losses)
    np.save(os.path.join(output_folder, f'val_accuracies{suffix}.npy'), val_accuracies)
    
    print("\n" + "="*60)
    print("训练完成！所有文件已保存")
    print("="*60)
    print(f"保存的文件(位于 {output_folder} 文件夹):")
